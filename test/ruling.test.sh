...
# The surface is a required caller fact. Both invalid shapes fail before any
# endpoint can be read, so they cannot post even if a fixture appears later.
check "a missing ruling surface fails loudly" 2 "surface" \
  reconcile_ruling 8 "$NOW" "$NOW"
check "an unrecognised ruling surface fails loudly" 2 "must be issue or pr" \
  reconcile_ruling 8 "$NOW" "$NOW" discussion
check "invalid ruling surfaces post nothing" 0 "0" posts 8
...
jq -n --arg at "$(iso "$T")" \
  ...
  >"$(timeline_file 9)"
printf '[]\n' >"$(comments_file 9)"
check "a bare flag is commented on" 0 "ruling flag is bare" reconcile_ruling 9 "$T" "$NOW" issue
check "...and the sweep never posted twice" 0 "" reconcile_ruling 9 "$T" "$NOW" issue
...
jq -n --arg at "$(iso "$((T - 840))")" --arg b "$TPL_BOLD" \
  ...
  >"$(comments_file 10)"
reconcile_ruling 10 "$T" "$NOW" issue >/dev/null
...
jq -n --arg esc "$(iso "$((T1 - 60))")" --arg marked "$(iso "$((T1 + 300))")" \
  ...
  >"$(comments_file 11)"
reconcile_ruling 11 "$T2" "$NOW" issue >/dev/null
...
jq -n --arg at "$(iso "$((T0 - 60))")" --arg b "$TPL" \
  ...
  >"$(comments_file 12)"
check "8 quiet days nudge" 0 "ruling nudge" reconcile_ruling 12 "$T0" "$NOW" issue
...
reconcile_ruling 12 "$(date -d "$newest_at" +%s)" "$NOW" issue >/dev/null
...
jq -n --arg at "$(iso "$((NOW - 3660))")" --arg b "$TPL" \
  ...
  >"$(comments_file 13)"
reconcile_ruling 13 $((NOW - 6 * 86400)) "$NOW" issue >/dev/null
...
check "an unreadable timeline invents no verdict" 0 "timeline unreadable" \
  reconcile_ruling 14 "$T" "$NOW" issue
...
check "a malformed escalation is commented on" 0 "escalation malformed" \
  reconcile_ruling 15 "$T15" "$NOW" issue
reconcile_ruling 15 "$T15" "$NOW" issue >/dev/null
...
check "the 12h rung fires despite recent activity — rungs never reset" 0 "12h rung" \
  reconcile_ruling 16 $((NOW - 60)) "$NOW" issue
reconcile_ruling 16 $((NOW - 60)) "$NOW" issue >/dev/null
...
# The 12h body is surface-agnostic. Drive the same shaped hard block through
# the PR surface and pin the posted bytes, not merely selected phrases.
jq -n --arg at "$(iso "$T16")" \
  '[{"event":"labeled","label":{"name":"needs-ruling"},"actor":{"login":"setter"},"created_at":$at}]' \
  >"$(timeline_file 116)"
jq -n --arg at "$(iso "$((T16 - 60))")" --arg b "$TPL_BOLD" \
  '[{"user":{"login":"setter"},"created_at":$at,"html_url":"https://x/esc116","body":$b}]' \
  >"$(comments_file 116)"
reconcile_ruling 116 $((NOW - 60)) "$NOW" pr >/dev/null
check "the 12h rung body is byte-identical on issue and PR surfaces" 0 "" \
  cmp "$TMP/posted-16" "$TMP/posted-116"
...
jq -n --arg at "$(iso "$((L17 - 60))")" --arg b "$TPL" \
  ...
  >"$(comments_file 17)"
reconcile_ruling 17 "$L17" $((L17 + 13 * 3600)) issue >/dev/null
...
check "a sweep at 25h posts the 24h rung — one comment per rung" 0 "24h rung" \
  reconcile_ruling 17 "$L17" "$NOW" issue
...
check "a timed issue default keeps the builder late-rung instruction" 0 "" \
  grep -qF 'proceeds regardless' "$TMP/posted-17"
reconcile_ruling 17 "$L17" "$NOW" issue >/dev/null
check "...and never a third within the episode" 0 "2" posts 17

# -- issue hard block: the late-rung picks are withdrawn, but the setter's
# re-read and the decider's nudge remain visible duties ---------------------
T117=$((NOW - 25 * 3600))
jq -n --arg at "$(iso "$T117")" \
  '[{"event":"labeled","label":{"name":"needs-ruling"},"actor":{"login":"setter"},"created_at":$at}]' \
  >"$(timeline_file 117)"
jq -n --arg at "$(iso "$((T117 - 60))")" --arg b "$TPL_BOLD" \
  '[{"user":{"login":"setter"},"created_at":$at,"html_url":"https://x/esc117","body":$b}]' \
  >"$(comments_file 117)"
check "an issue hard block posts the carved-out rung-24 reminder" 0 \
  "late rungs do not fire" reconcile_ruling 117 "$T117" "$NOW" issue
check "the carved-out body addresses and names the flag-setter" 0 "" \
  grep -qF 'flag-setter, @setter' "$TMP/posted-117"
check "the carved-out body asks for a published re-read against what landed" 0 "" \
  grep -qF 'published re-read of the default against what has landed' "$TMP/posted-117"
check "the carved-out body says the flag waits for the human" 0 "" \
  grep -qF 'the flag waits for the human' "$TMP/posted-117"
check "the carved-out body reports the hard block as before" 0 "" \
  grep -qF "$PR_HARD_DESC" "$TMP/posted-117"
check "the carved-out body names the remaining decider nudge" 0 "" \
  grep -qF "decider's 7-day nudge" "$TMP/posted-117"
check "the carved-out body contains none of the withdrawn acts" 1 "" \
  grep -qE 'proceeds regardless|as a PR|triage picks the option|there is no further timer' \
  "$TMP/posted-117"
...
# The same hard block on a PR keeps the existing full ladder byte-for-byte.
jq -n --arg at "$(iso "$T117")" \
  '[{"event":"labeled","label":{"name":"needs-ruling"},"actor":{"login":"setter"},"created_at":$at}]' \
  >"$(timeline_file 118)"
jq -n --arg at "$(iso "$((T117 - 60))")" --arg b "$TPL_BOLD" \
  '[{"user":{"login":"setter"},"created_at":$at,"html_url":"https://x/esc118","body":$b}]' \
  >"$(comments_file 118)"
reconcile_ruling 118 "$T117" "$NOW" pr >/dev/null
check "the posted PR hard-block body matches the merge-base golden" 0 "" \
  diff -u "$ROOT/test/fixtures/ruling-rung24-pr.golden.md" \
  <(sed '/^----$/d' "$TMP/posted-118")
...
jq -n --arg at "$(iso "$((T18 - 60))")" \
  ...
  >"$(comments_file 18)"
reconcile_ruling 18 "$T18" "$NOW" issue >/dev/null
...
check "an unparseable issue default takes the carved-out branch" 0 "" \
  grep -qF 'late rungs do not fire' "$TMP/posted-18"
check "an unparseable issue default contains none of the withdrawn acts" 1 "" \
  grep -qE 'proceeds regardless|as a PR|triage picks the option' "$TMP/posted-18"
...
jq -n --arg esc "$(iso "$((TB - 60))")" --arg b "$TPL" \
  ...
  >"$(comments_file 19)"
reconcile_ruling 19 "$TB" "$NOW" issue >/dev/null
...
check "an unreadable comment list invents no verdict" 0 "comments unreadable" \
  reconcile_ruling 20 "$T" "$NOW" issue
...
jq -n --arg at "$(iso "$((T21 - 60))")" \
  ...
  >"$(comments_file 21)"
reconcile_ruling 21 "$T21" "$NOW" issue >/dev/null
