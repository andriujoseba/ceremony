...
6. **`.github/workflows/refs-guard.yml`** — the body-aware guard is its own
   ...
   `refs-not-closing` is available at `0.6.0` and later (#218).
...
`RELEASES.md` joins that mirror at `0.6.0` and later (#248): consumers add
...
complete set as of that tag. That guarantee is available at `0.6.0` and
later (#251); the manifest is worth reading at every earlier pin regardless,
since it is what `actions/docs-sync` has always mirrored.
