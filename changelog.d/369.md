...
- A disposal whose flag reads back `archived: false` for the whole retry
  ...
  absence of one (#369).
- A drill probe no longer compares two fragment lists neither of which was
  read: the `changelog.d` listing reports a failed read as a failure, and an
  empty list now means only what it says — a directory a promotion has
  consumed (#369).
