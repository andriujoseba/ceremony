...
- `runner-isolated` reads a self-hosted label passed through a `with:` input,
  ...
  exactly as one named in `runs-on:` (#395). Either is read in whatever
  spelling it is written: quoted, flow, block scalar, a flow collection
  opening on the line after its key, or an alias to an anchor the same file
  defines (#395).
